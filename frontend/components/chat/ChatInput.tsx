...
export function ChatInput({ onSendMessage, isLoading }: ChatInputProps) {
  ...
  const handleSend = () => {
    // Chỉ gửi khi nội dung (sau khi xóa khoảng trắng) không rỗng và AI không đang trả lời
  ...
  return (
    ...
    <div className="p-4 bg-transparent mt-auto relative z-10 w-full max-w-4xl mx-auto">
      ...
      <div className="relative flex items-end gap-2 bg-[#F1CCA6]/40 p-2 pl-4 pr-2 rounded-3xl border border-[#C1762A]/20 shadow-sm focus-within:border-[#C1762A] focus-within:bg-white transition-all">
        
        {/* Nút đính kèm file (Hiện tại chỉ là giao diện giả lập, chưa xử lý chức năng tải lên) */}
        <button className="p-2.5 text-[#8C4905] hover:bg-[#F1CCA6] rounded-full transition-colors self-end mb-0.5">
          <Paperclip size={20} />
        </button>
        ...
        <textarea
          ...
          // CSS khóa resize thủ công (resize-none), cho phép thanh cuộn dọc nếu vượt max-h-[150px]
          className="flex-1 max-h-[150px] min-h-[44px] py-3 bg-transparent text-[#000000] placeholder:text-[#8C4905]/60 outline-none resize-none overflow-y-auto font-medium"
          rows={1}
          disabled={isLoading}
        />
        ...
        <button
          ...
          // Sử dụng hàm cn() (từ clsx, tailwind-merge) để tùy biến class khi nút bị vô hiệu hóa
          className={cn(
        ...
        >
          <Send size={20} className={cn(content.trim() && "ml-0.5")} />
        </button>
      </div>
