...
import { LogOut, Settings, X, Plus, Edit2, Trash2 } from "lucide-react";
...
const mockHistory = [
  { id: "h1", title: "a/an excercise", date: "Today" },
  { id: "h2", title: "a/an excercise", date: "Yesterday" },
  { id: "h3", title: "a/an excercise", date: "dd Month" },
  { id: "h4", title: "a/an excercise", date: "dd Month" },
];

export function ChatSidebar({ className, isMobileOpen, setIsMobileOpen }: ChatSidebarProps) {
  const router = useRouter();
  const { data: session } = useSession();
  const [activeChatId, setActiveChatId] = useState("h1");
    ...
    <>
      ...
      >
        ...
        <div className="p-4 flex flex-col gap-4">
          <div className="flex items-center gap-3 mb-2 cursor-pointer" onClick={() => router.push('/dashboard')}>
            <div className="w-10 h-10 bg-[#D9D9D9] rounded-xl flex items-center justify-center text-[#8C4905] font-bold text-sm shadow-inner">
              SK
            </div>
            <h2 className="font-extrabold italic text-xl text-[#8C4905] tracking-tight">SocraticKid</h2>
          </div>
                  ...
                  >
                    ...
                    {activeChatId === chat.id && (
                      <div className="flex gap-2 text-[#C1762A]">
                        <button className="hover:text-[#8C4905]"><Edit2 size={14} /></button>
                        <button className="hover:text-[#8C4905]"><Trash2 size={14} /></button>
                      </div>
                    )}
                  </div>
                ))}
